...
def plot_projected_gravity(json_file):
    ...
    # Create title with config info
    config_title = f"{mode} - {input_type} of {amplitude} for {duration} seconds ({collected_on})"
    ...
    ax1.plot(times_np, act_pos_np, label='Actuator Position', linewidth=2, color='purple')
    ...
    ax2.plot(times_np, expected_X, label='Expected X', linestyle='--', color=colors[0], linewidth=3)
    # ax2.plot(times_np, expected_Y, label='Expected Y', linestyle='--', color=colors[1], linewidth=3)
    ax2.plot(times_np, expected_Z, label='Expected Z', linestyle='--', color=colors[2], linewidth=3)
    ...
    ax2.text(0.80, 0.75, textstr, transform=ax2.transAxes, fontsize=10,
