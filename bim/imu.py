...
async def sample_loop(kos: KOS, imu_kos: KOS, DATA: List[float], duration: float, actuator_id: int):
    ...
    while time.monotonic() < end:
        ...
        if 1.0 / sampling_rate - (time.monotonic() - loop_t0) > 0.0:
            logger.warning(f"Loop overran by {1.0 / sampling_rate - (time.monotonic() - loop_t0)} seconds")
...
async def run_test(
    ...
    sampler = asyncio.create_task(sample_loop(kos, imu_kos, DATA, duration, actuator_id))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_type", type=str, choices=["step", "chirp"], default="step")
    parser.add_argument("--title", type=str, default=datetime.now().strftime("%Y%m%d_%H%M%S"))
